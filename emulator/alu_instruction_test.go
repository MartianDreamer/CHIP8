...
func Test_set_vx_d_timer(t *testing.T) {
	...
	emulator.exec_opcode_f_ins([2]byte{0xf1, 0x07})
...
func Test_set_d_timer_vx(t *testing.T) {
	emulator := Make_chip8(500)
	emulator.v[0x1] = 0xf1
	emulator.exec_opcode_f_ins([2]byte{0xf1, 0x15})
...
func Test_set_s_timer_vx(t *testing.T) {
	emulator := Make_chip8(500)
	emulator.v[0x1] = 0xf1
	emulator.exec_opcode_f_ins([2]byte{0xf1, 0x18})
...
func Test_set_i_i_add_vx(t *testing.T) {
	...
	emulator.i = 100
	emulator.v[0x2] = 200
	emulator.exec_opcode_f_ins([2]byte{0xf2, 0x1e})
	if emulator.i != 300 {
...
func Test_set_i_font_location(t *testing.T) {
	emulator := Make_chip8(500)
	emulator.v[0x2] = 0xd
	emulator.exec_opcode_f_ins([2]byte{0xf2, 0x29})
	if emulator.i != 65 {
		t.Fatalf("i: %d", emulator.i)
	}
}
...
func Test_bcd(t *testing.T) {
	...
	emulator.v[0x2] = 203
	emulator.i = 0x300
	emulator.exec_opcode_f_ins([2]byte{0xf2, 0x33})
...
func Test_store_register(t *testing.T) {
	...
	emulator.v[0x3] = 0x25
	emulator.i = 0x200
	emulator.exec_opcode_f_ins([2]byte{0xf3, 0x55})
...
func Test_read_register(t *testing.T) {
	...
	emulator.i = 300
	emulator.exec_opcode_f_ins([2]byte{0xf3, 0x65})
...
func Test_set_vx_key(t *testing.T) {
	emulator := Make_chip8(500)
	emulator.pc = 0x215
	emulator.exec_opcode_f_ins([2]byte{0xf0, 0x0a})
